...
int main() 
{
    ...
    // 生成协商密钥对
	OQS_STATUS rc = OQS_KEM_keypair(kem, pkn, skn);
	if (rc != OQS_SUCCESS) {
    ...
    // 接收bob的签名公钥pksb
    memset(buffer, 0, sizeof(buffer));
    if(!ReadSocket(sock, buffer, sig_a->length_public_key)){
    ...
    pkn_timestamp[1191] = timestamp_1 && 0xff;
    pkn_timestamp[1190] = (timestamp_1 && 0xff00) >> 8;
    pkn_timestamp[1189] = (timestamp_1 && 0xff0000) >> 8 * 2;
    pkn_timestamp[1188] = (timestamp_1 && 0xff000000) >> 8 * 3;
    pkn_timestamp[1187] = (timestamp_1 && 0xff00000000) >> 8 * 4;
    pkn_timestamp[1186] = (timestamp_1 && 0xff0000000000) >> 8 * 5;
    pkn_timestamp[1185] = (timestamp_1 && 0xff000000000000) >> 8 * 6;
    pkn_timestamp[1184] = (timestamp_1 && 0xff00000000000000) >> 8 * 7;
	...
	if (rc != OQS_SUCCESS) {
		fprintf(stderr, "ERROR: Signature of pkn and timestamp generating failed!\n");
    ...
    signed_message_1 = malloc(sig_a->length_signature + kem->length_public_key + sizeof(uint64_t));
    ...
    // 接收signed_message_2
    memset(buffer, 0, sizeof(buffer));
    if(!ReadSocket(sock, buffer, sig_a->length_signature + kem->length_ciphertext + sizeof(uint64_t))){
    ...
    if(receive_signed_message_2 == NULL){
        fprintf(stderr, "ERROR: malloc of receive_signed_message_2 failed!\n");
        return OQS_ERROR;
    }
    ...
    if(receive_ciphertext_timestamp == NULL){
        fprintf(stderr, "ERROR: malloc of receive_ciphertext_timestamp failed!\n");
        return -1;
    }
    ...
    if(receive_signature_2 == NULL){
        fprintf(stderr, "ERROR: malloc of receive_signature_2 failed!\n");
        return -1;
    }
    ...
    receive_timestamp_2 = *(receive_ciphertext_timestamp + kem->length_ciphertext);
    ...
    // Alice成功获得共享密钥shared_secret
    printf("Alice successfully generates shared_secrect.\n");

    // 若程序运行至此，说明Alice和Bob已经完成了双向身份认证，并完成了共享密钥的协商
    printf("So far, Alice and Bob have completed the two-way identity authentication and the negotiation of the shared_secret, and the 'YYH Protocal' is complete!");
