...
int main() 
{
    ...
    // 接收alice的公钥pksa
    memset(buffer, 0, sizeof(buffer));
    if(!ReadSocket(new_socket, buffer, sig_b->length_public_key)){
    ...
    memset(buffer, 0, sizeof(buffer));
    if(!ReadSocket(new_socket, buffer, length_signed_message_1)){
    ...
    receive_timestamp = *(receive_pkn_timestamp + kem->length_public_key);
    ...
    if(rc != OQS_SUCCESS){
        fprintf(stderr, "Signature of ciphertext_timestamp generating failed!\n");
        return OQS_ERROR;
    }
    ...
    send(server_fd, signed_message_2, sig_b->length_signature + kem->length_ciphertext + sizeof(uint64_t), 0);
